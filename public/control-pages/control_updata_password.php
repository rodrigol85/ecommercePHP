...
$searchEmail = $user->findByEmail($email);

if(empty($searchEmail)){
    session_start();
    $_SESSION['emailNonTrovato'] = "Questa email non è  presente nel database";
    header("Location: ../?page=new_password");
    exit;
}else{
    $confirm_code = bin2hex(random_bytes(32));
    $updateToken = $db->updatetokenUser($email, $confirm_code);
    $link = 'http://localhost/ecommerce/public/?page=reset_password&token=' . $confirm_code;
