...
class Database
{
    ...
    public function updatetokenUser($email, $confirm_code)
    {
        $stmt = $this->conn->prepare("UPDATE `users` SET `confirm_code` = :confirm_code  WHERE `email` = :email");
        $stmt->bindValue(':email', $email, PDO::PARAM_STR);
        $stmt->bindValue(':confirm_code', $confirm_code, PDO::PARAM_STR);
    ...
    public function getTotalQuantity($chart_id)
    {
        $stmt = $this->conn->prepare("SELECT SUM(quantity) as total_quantity FROM chart_items WHERE chart_id = :chart_id");
        $stmt->bindValue(':chart_id', $chart_id, PDO::PARAM_INT);
        $stmt->execute();
    ...
    public function updatetokenAdmin($email, $confirm_code)
    {
        $stmt = $this->conn->prepare("UPDATE `admins` SET `confirm_code` = :confirm_code  WHERE `email` = :email");
        $stmt->bindValue(':email', $email, PDO::PARAM_STR);
        $stmt->bindValue(':confirm_code', $confirm_code, PDO::PARAM_STR);
    ...
    public function updatePasswordAdmin($email, $password)
    {
        $pass = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $this->conn->prepare("UPDATE `admins` SET `password` = :password WHERE `email` = :email");
        $stmt->bindValue(':email', $email, PDO::PARAM_STR);
        $stmt->bindValue(':password', $pass, PDO::PARAM_STR);
    ...
    public function findByTokenAdmin($confirm_code)
    {
        $stmt = $this->conn->prepare("SELECT * FROM admins WHERE confirm_code = :confirm_code");
        $stmt->bindValue(':confirm_code', $confirm_code, PDO::PARAM_STR);
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$row) {
